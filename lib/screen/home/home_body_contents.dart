import 'package:first_challenge/screen/home/feeds.dart';
...
class Home_Page extends StatefulWidget {
  ...
  static final List<String> _categories = [
    'All', 'Diet Type', 'Day Time', 'Preparation Time', 'Drink Type', 'More'
  ];
  ...
  Widget build(BuildContext context) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start,
      children: <Widget>[
        ...
        const Text('What would you like to cook?', style: TextStyle(
        ...
        Text('Categories', style: Theme.of(context).textTheme.subtitle1?.merge(const TextStyle(color: black_10)),),
        categories_items(),
        const SizedBox(height: 16,),
        Text('Popular Recipes', style: Theme.of(context).textTheme
            .subtitle1?.merge(const TextStyle(color: black_10)),),
        const SizedBox(height: 8,),
        Expanded(
                ...
                SizedBox(
                  ...
                  child: ListView.separated(
                    ...
                    itemBuilder: (context,index)=>Container(
                      height: 237,width: 190,
                      decoration: BoxDecoration(
                        image: DecorationImage(image: AssetImage(items[index].image)),
                        borderRadius: BorderRadius.circular(8),
                      ),
                      child: buildCard(content: items[index].content, image:
                      items[index].image,
      ...
      child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween,
        children: [
                ...
                child: Container(
                    ...
                    child: Padding(
                      padding: const EdgeInsets.symmetric(vertical: 3.0,
                          horizontal: 18.0),
                      child: Center(
                          child: Text(Home_Page._categories[index],textAlign:
                          TextAlign.center, style: Theme.of(context).textTheme
                              .subtitle1?.merge(const TextStyle(fontSize: 9, color:
                          black_10)),)),
                    )),
              ),
            ),
          ...
          Container(
            ...
            child: InkWell(child: Center(child: SvgPicture.asset('assets/icons/list_forward.svg'))),
          ),
        ],
      ),
    );
...
class CardItem{
  ...
  const CardItem({required this.image, required this.content, required this
      .time});
}
              ...
              child:   Column(crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Row(mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      Text(content, style: Theme.of(context).textTheme
                          .button?.merge(const TextStyle(fontSize: 13,color: Color(0xffFFFFFF) )),),
                      Image.asset('assets/images/saved.png'),
                    ],
                  ),
                  const SizedBox(height: 8,),
                  Text(time, style: Theme.of(context).textTheme
                      .button?.merge(const TextStyle(fontWeight: FontWeight.w400,color: Color
                    (0xffFFFFFF) )),),
                ],),
            )
        ),
